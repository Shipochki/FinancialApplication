...
export class GlobalAuthService {
  ...
  getUserEmail(): string | null {
    // Get all accounts known to MSAL
    const accounts = this.msalService.instance.getAllAccounts();

    if (accounts.length > 0) {
      // Typically, the first account is the active one,
      // or you can use this.msalService.instance.getActiveAccount() if you set it during login.
      const account = accounts[0];

      // Azure AD usually stores the email in the 'username' property (UPN)
      // or inside the idTokenClaims as 'preferred_username' or 'email'
      return (
        account.username ||
